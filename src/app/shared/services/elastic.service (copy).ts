...
export class ElasticService {
    ...
    getTableDataByRawQueryFields(databaseName: string, tableName: string, rawQueryString: string, fieldsToFetch: string, offset: number, limit: number, sort: string = 'Desc') {
        let headers = new Headers();
        headers.append('Content-Type', 'application/json');
        return this._http.get(this._url + '/platform/getAll/ELASTIC_CLIENT' + '/' + databaseName + '/' + tableName + '?offset=' + offset + '&limit=' + limit + '&sortOrder=' + sort + '&rawQuery=' + rawQueryString + '&fieldsToFetch=' + fieldsToFetch, {headers: headers})
            .map((res: Response) => ElasticService.json(res))
            .catch(this.handleError);
